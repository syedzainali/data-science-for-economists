...
possibly_download_html = purrr::possibly(download.file, otherwise=NA)
...
for (i in 1:nrow(regions)) {
    ...
    if (!file.exists(file_path)) {
      possibly_download_html(str_c(regions[i]$url, "search/products/?q=00263850"), destfile = file_path)
    }
